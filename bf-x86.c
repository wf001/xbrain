...
void program_strip_nops(struct program *p) {
...
void optimize_replace(struct program *p, long i, struct optimization *o,
                      long *vars) {
...
long optimize_apply(struct program *p, struct optimization *o) {
    long count = 0;
    long variables[16] = {0};
    for (size_t i = 0; i < p->count; i++) {
        long length = 0;
        for (size_t n = 0; o->sequence[n].ins != INS_NOP && i + n < p->count;
             n++) {
            ...
            switch (o_type) {
                case OP_CONSTANT:
                    if (operand != o_operand) length = 0;
                    break;
                case OP_STORE:
                    variables[o_operand] = operand;
                    break;
                case OP_LOAD:
                    if (operand != variables[o_operand]) length = 0;
                    break;
                case OP_LOAD_NEG:
                    if (operand != -variables[o_operand]) length = 0;
                    break;
    ...
    if (count > 0) program_strip_nops(p);
    return count;
}
...
struct optimization optimization_clear = {
    ...
    .sequence =
        (struct pattern[]){
            {INS_BRANCH}, {INS_MUTATE, OP_CONSTANT, -1}, {INS_JUMP}, {INS_NOP}},
    .replacement = (struct pattern[]){{INS_CLEAR}, {INS_NOP}, {INS_NOP}}};

/* [->+<] */
struct optimization optimization_add = {
    .name = "Cell add",
    .sequence = (struct pattern[]){{INS_BRANCH},
                                   ...
                                   {INS_JUMP},
                                   {INS_NOP}},
    .replacement = (struct pattern[]){{INS_ADD, OP_LOAD, 0},
                                      {INS_CLEAR},
                                      {INS_NOP},
                                      {INS_NOP},
                                      {INS_NOP},
...
void program_compress(struct program *p) {
    for (size_t i = 0; i < p->count; i++) {
        if (p->ins[i].ins == INS_MOVE || p->ins[i].ins == INS_MUTATE) {
            ...
            if (p->ins[i].operand == 0) p->ins[i].ins = INS_NOP;
        }
    }
    program_strip_nops(p);
}

void program_optimize(struct program *p, int level) {
    if (level == 0) return;
    program_compress(p);
    long count = 0;
    do {
        count = 0;
        if (level >= 2) count += optimize_apply(p, &optimization_clear);
        if (level >= 3) count += optimize_apply(p, &optimization_add);
    } while (count > 0);
}

void program_print(const struct program *p, FILE *out) {
    for (size_t i = 0; i < p->count; i++) {
...
void interpret(const struct program *program, FILE *trace) {
    long ip = 0;
    long dp = 0;
...
int main(int argc, char **argv) {
    ...
    if (do_debug) program_print(&program, stderr);
